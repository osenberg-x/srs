...
int SrsRtpConn::port() { return _port; }

srs_error_t SrsRtpConn::listen() { return listener->listen(); }

srs_error_t SrsRtpConn::on_udp_packet(const sockaddr* from, const int fromlen,
                                      char* buf, int nb_buf) {
  ...
  if (true) {
    ...
    if (pkt.chunked) {
      ...
      if (pprint->can_print()) {
        srs_trace("<- " SRS_CONSTS_LOG_STREAM_CASTER
                  " rtsp: rtp chunked %dB, age=%d, vt=%d/%u, sts=%u/%#x/%#x, "
                  "paylod=%dB",
                  nb_buf, pprint->age(), cache->version, cache->payload_type,
                  cache->sequence_number, cache->timestamp, cache->ssrc,
  ...
  if (pprint->can_print()) {
    srs_trace("<- " SRS_CONSTS_LOG_STREAM_CASTER
              " rtsp: rtp #%d %dB, age=%d, vt=%d/%u, sts=%u/%u/%#x, "
              "paylod=%dB, chunked=%d",
              stream_id, nb_buf, pprint->age(), cache->version,
              cache->payload_type, cache->sequence_number, cache->timestamp,
              cache->ssrc, cache->payload->length(), cache->chunked);
  }
...
int64_t SrsRtspJitter::timestamp() { return pts; }
...
std::string SrsRtspConn::desc() { return "RtspConn"; }

const SrsContextId& SrsRtspConn::get_id() { return _srs_context->get_id(); }
...
srs_error_t SrsRtspConn::do_cycle() {
  ...
  while (true) {
    ...
    if (req->is_options()) {
      ...
      srs_trace(
          "rtsp: video(#%d, %s, %s/%s), audio(#%d, %s, %s/%s, %dHZ "
          "%dchannels), %s/%s",
          video_id, video_codec.c_str(), req->sdp->video_protocol.c_str(),
          req->sdp->video_transport_format.c_str(), audio_id,
          audio_codec.c_str(), req->sdp->audio_protocol.c_str(),
          req->sdp->audio_transport_format.c_str(), audio_sample_rate,
          audio_channel, rtsp_tcUrl.c_str(), rtsp_stream.c_str());
      ...
      srs_trace(
          "rtsp: #%d %s over %s/%s/%s %s client-port=%d-%d, server-port=%d-%d",
          req->stream_id, (req->stream_id == video_id) ? "Video" : "Audio",
          req->transport->transport.c_str(), req->transport->profile.c_str(),
          req->transport->lower_transport.c_str(),
          req->transport->cast_type.c_str(), req->transport->client_port_min,
          req->transport->client_port_max, lpm, lpm + 1);
...
srs_error_t SrsRtspConn::cycle() {
  ...
  if (err == srs_success) {
    ...
    srs_warn("rtsp: client disconnect peer. code=%d", srs_error_code(err));
    srs_freep(err);
  }
...
srs_error_t SrsRtspConn::on_rtp_video(SrsRtspPacket* pkt, int64_t dts,
                                      int64_t pts) {
  srs_error_t err = srs_success;
...
srs_error_t SrsRtspConn::kickoff_audio_cache(SrsRtspPacket* pkt, int64_t dts) {
  ...
  if (dts - acache->dts > 0 && acache->audio->nb_samples > 0) {
    ...
    for (int i = 0; i < acache->audio->nb_samples; i++) {
      ...
      if ((err = write_audio_raw_frame(frame, nb_frame, acodec,
                                       (uint32_t)timestamp)) != srs_success) {
...
srs_error_t SrsRtspConn::write_sequence_header() {
  ...
  if ((err = format->on_aac_sequence_header((char*)sh.c_str(),
                                            (int)sh.length())) != srs_success) {
  ...
  acodec->sound_type =
      (dec->aac_channels == 2) ? SrsAudioChannelsStereo : SrsAudioChannelsMono;
  ...
  if ((err = write_audio_raw_frame((char*)sh.data(), (int)sh.length(), acodec,
                                   (uint32_t)dts)) != srs_success) {
    return srs_error_wrap(err, "write audio raw frame");
  }
...
srs_error_t SrsRtspConn::write_h264_sps_pps(uint32_t dts, uint32_t pts) {
  ...
  if (h264_sps.empty() || h264_pps.empty()) {
    srs_warn("no sps=%dB or pps=%dB", (int)h264_sps.size(),
             (int)h264_pps.size());
    return err;
  }
...
srs_error_t SrsRtspConn::write_h264_ipb_frame(char* frame, int frame_size,
...
srs_error_t SrsRtspConn::write_audio_raw_frame(char* frame, int frame_size,
...
srs_error_t SrsRtspConn::rtmp_write_packet(char type, uint32_t timestamp,
                                           char* data, int size) {
  srs_error_t err = srs_success;
...
srs_error_t SrsRtspConn::connect() {
  ...
  if (!req) {
    ...
    int port;
    srs_discovery_tc_url(rtsp_tcUrl, schema, host, vhost, app, rtsp_stream,
                         port, param);
...
void SrsRtspConn::close() { srs_freep(sdk); }
