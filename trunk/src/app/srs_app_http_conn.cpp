...
SrsHttpConn::SrsHttpConn(ISrsHttpConnOwner* handler, ISrsProtocolReadWriter* fd,
                         ISrsHttpServeMux* m, string cip, int cport) {
...
std::string SrsHttpConn::desc() { return "HttpConn"; }

void SrsHttpConn::remark(int64_t* in, int64_t* out) { kbps->remark(in, out); }
...
srs_error_t SrsHttpConn::cycle() {
  ...
  // success.
  if (err == srs_success) {
    srs_trace("http: client finished.");
    return err;
  }

  // It maybe success with message.
  if (srs_error_code(err) == ERROR_SUCCESS) {
    srs_trace("http: client finished%s.", srs_error_summary(err).c_str());
    srs_freep(err);
    return err;
  }
  ...
  if (srs_is_client_gracefully_close(err)) {
    ...
    srs_warn("http: server disconnect. ret=%d", srs_error_code(err));
  } else {
...
srs_error_t SrsHttpConn::process_request(ISrsHttpResponseWriter* w,
                                         ISrsHttpMessage* r, int rid) {
  srs_error_t err = srs_success;

  srs_trace("HTTP #%d %s:%d %s %s, content-length=%" PRId64 "", rid, ip.c_str(),
            port, r->method_str().c_str(), r->url().c_str(),
            r->content_length());
...
srs_error_t SrsHttpConn::pull() { return trd->pull(); }
...
string SrsHttpConn::remote_ip() { return ip; }

const SrsContextId& SrsHttpConn::get_id() { return trd->cid(); }

void SrsHttpConn::expire() { trd->interrupt(); }

SrsResponseOnlyHttpConn::SrsResponseOnlyHttpConn(bool https,
                                                 ISrsResourceManager* cm,
                                                 srs_netfd_t fd,
                                                 ISrsHttpServeMux* m,
                                                 string cip, int port) {
...
srs_error_t SrsResponseOnlyHttpConn::pop_message(ISrsHttpMessage** preq) {
  ...
  while (true) {
    ...
    if ((err = io->read(body, 4096, NULL)) != srs_success) {
      // Because we use timeout to check trd state, so we should ignore any
      // timeout.
...
srs_error_t SrsResponseOnlyHttpConn::on_http_message(ISrsHttpMessage* r,
...
srs_error_t SrsResponseOnlyHttpConn::on_message_done(ISrsHttpMessage* r,
                                                     SrsHttpResponseWriter* w) {
  return srs_success;
}
...
std::string SrsResponseOnlyHttpConn::remote_ip() { return conn->remote_ip(); }

const SrsContextId& SrsResponseOnlyHttpConn::get_id() { return conn->get_id(); }
...
srs_error_t SrsHttpServer::initialize() {
  ...
  // for SRS go-sharp to detect the status of HTTP server of SRS HTTP FLV
  // Cluster.
  if ((err = http_static->mux.handle("/api/v1/versions",
                                     new SrsGoApiVersion())) != srs_success) {
...
srs_error_t SrsHttpServer::serve_http(ISrsHttpResponseWriter* w,
                                      ISrsHttpMessage* r) {
  srs_error_t err = srs_success;
