...
SrsPsRtpPacket::SrsPsRtpPacket() { isFirstPacket = false; }
...
srs_error_t SrsPsRtpPacket::decode(SrsBuffer* stream) {
  ...
  if (!stream->require(12)) {
    return srs_error_new(ERROR_RTP_HEADER_CORRUPT, "requires 12 only %d bytes",
                         stream->left());
  }
  ...
  if (payload_type == 96) {
    ...
    if (!stream->require(1)) {
      return srs_error_new(ERROR_RTP_TYPE96_CORRUPT, "requires 1 only %d bytes",
                           stream->left());
    }
    // append left bytes to payload.
    payload->append(stream->data() + stream->pos(),
...
int SrsPsRtpListener::port() { return _port; }

srs_error_t SrsPsRtpListener::listen() { return listener->listen(); }

srs_error_t SrsPsRtpListener::on_udp_packet(const sockaddr* from,
                                            const int fromlen, char* buf,
                                            int nb_buf) {
  srs_error_t err = srs_success;
  if (rtp_processor) {
...
srs_error_t SrsGb28181RtpMuxService::on_udp_packet(const sockaddr* from,
                                                   const int fromlen, char* buf,
                                                   int nb_buf) {
  srs_error_t err = srs_success;
  if (rtp_processor) {
...
SrsGb28181PsRtpProcessor::SrsGb28181PsRtpProcessor(SrsGb28181Config* c,
                                                   std::string id) {
  config = c;
...
void SrsGb28181PsRtpProcessor::dispose() {
  ...
  for (it2 = cache_ps_rtp_packet.begin(); it2 != cache_ps_rtp_packet.end();
       ++it2) {
...
srs_error_t SrsGb28181PsRtpProcessor::on_udp_packet(const sockaddr* from,
                                                    const int fromlen,
                                                    char* buf, int nb_buf) {
...
srs_error_t SrsGb28181PsRtpProcessor::on_tcp_packet(const sockaddr* from,
                                                    const int fromlen,
                                                    char* buf, int nb_buf) {
  return on_udp_packet(from, fromlen, buf, nb_buf);
}

srs_error_t SrsGb28181PsRtpProcessor::on_rtp_packet(const sockaddr* from,
                                                    const int fromlen,
                                                    char* buf, int nb_buf) {
  srs_error_t err = srs_success;
  ...
  char port_string[16];
  if (getnameinfo(from, fromlen, (char*)&address_string, sizeof(address_string),
  ...
  if (true) {
    ...
    // TODO: fixme: the same device uses the same SSRC to send with different
    // local ports
    std::stringstream ss;
    ...
    if (((pre_sequence_number + 1) % 65536) != pkt.sequence_number &&
      ...
      srs_warn(
          "gb28181: ps sequence_number out of order, ssrc=%#x, pre=%u, cur=%u, "
          "peer(%s, %s)",
          pkt.ssrc, pre_sequence_number, pkt.sequence_number, address_string,
          port_string);
      // return err;
    }
    ...
    if (pprint->can_print()) {
      srs_trace("<- " SRS_CONSTS_LOG_GB28181_CASTER
                " gb28181: client_id %s, peer(%s, %d) ps rtp packet %dB, "
                "age=%d, vt=%d/%u, sts=%u/%u/%#x, paylod=%dB",
                channel_id.c_str(), address_string, peer_port, nb_buf,
                pprint->age(), pkt.version, pkt.payload_type,
                pkt.sequence_number, pkt.timestamp, pkt.ssrc,
                pkt.payload->length());
    }
    ...
    map<std::string, SrsPsRtpPacket*>::iterator key =
        cache_ps_rtp_packet.find(pkt_key);
    if (key != cache_ps_rtp_packet.end()) {
      SrsGb28181RtmpMuxer* muxer = NULL;
      muxer = fetch_rtmpmuxer(channel_id, pkt.ssrc);

      if (muxer) {
        // TODO: fixme: the same device uses the same SSRC to send with
        // different local ports record the first peer port
        ...
        if (muxer->channel_peer_port() != peer_port) {
          srs_warn("<- " SRS_CONSTS_LOG_GB28181_CASTER
                   " gb28181: client_id %s, ssrc=%#x, first peer_port=%d cur "
                   "peer_port=%d",
                   muxer->get_channel_id().c_str(), pkt.ssrc,
                   muxer->channel_peer_port(), peer_port);
...
SrsGb28181RtmpMuxer* SrsGb28181PsRtpProcessor::fetch_rtmpmuxer(
    std::string channel_id, uint32_t ssrc) {
  if (true) {
    SrsGb28181RtmpMuxer* muxer = NULL;
    // First, search according to the channel_id. Otherwise, search according to
    // the SSRC. Some channel_id are created by RTP pool, which are different
    // ports. No channel_id are created by multiplexing ports, which are the
    // same port
    if (!channel_id.empty()) {
    ...
    if (!muxer && config->auto_create_channel) {
      ...
      if ((err2 = _srs_gb28181->create_stream_channel(&channel)) !=
          srs_success) {
        srs_warn("gb28181: RtpProcessor create stream channel error %s",
...
srs_error_t SrsGb28181PsRtpProcessor::rtmpmuxer_enqueue_data(
    SrsGb28181RtmpMuxer* muxer, uint32_t ssrc, int peer_port,
    std::string address_string, SrsPsRtpPacket* pkt) {
  srs_error_t err = srs_success;
  ...
  if (muxer) {
    // TODO: fixme: the same device uses the same SSRC to send with different
    // local ports record the first peer port
    muxer->set_channel_peer_port(peer_port);
    muxer->set_channel_peer_ip(address_string);
    // not the first peer port's non processing
    if (muxer->channel_peer_port() != peer_port) {
      srs_warn("<- " SRS_CONSTS_LOG_GB28181_CASTER
               " gb28181: client_id %s, ssrc=%#x, first peer_port=%d cur "
               "peer_port=%d",
               muxer->get_channel_id().c_str(), ssrc,
               muxer->channel_peer_port(), peer_port);
    } else {
...
SrsPsStreamDemixer::SrsPsStreamDemixer(ISrsPsStreamHander* h, std::string id,
                                       bool a, bool k) {
  hander = h;
  audio_enable = a;
...
SrsPsStreamDemixer::~SrsPsStreamDemixer() { srs_freep(aac); }

bool SrsPsStreamDemixer::can_send_ps_av_packet() {
  if (!wait_first_keyframe) return true;

  if (first_keyframe_flag) return true;
...
srs_error_t SrsPsStreamDemixer::on_ps_stream(char* ps_data, int ps_size,
                                             uint32_t timestamp,
                                             uint32_t ssrc) {
  srs_error_t err = srs_success;
  int complete_len = 0;
  ...
  while (incomplete_len > 0 && incomplete_len >= sizeof(SrsPsPacketStartCode)) {
    if (next_ps_pack && next_ps_pack[0] == (char)0x00 &&
        next_ps_pack[1] == (char)0x00 && next_ps_pack[2] == (char)0x01 &&
      ...
      next_ps_pack =
          next_ps_pack + sizeof(SrsPsPacketHeader) + pack_stuffing_length;
      complete_len =
          complete_len + sizeof(SrsPsPacketHeader) + pack_stuffing_length;
      incomplete_len = ps_size - complete_len;
    } else if (next_ps_pack && next_ps_pack[0] == (char)0x00 &&
               next_ps_pack[1] == (char)0x00 && next_ps_pack[2] == (char)0x01 &&
      ...
      while (es_map_length >= 4) {
        ...
        if (es_id >= PS_AUDIO_ID && es_id <= PS_AUDIO_ID_END) {
          if (audio_es_type != type) {
            srs_trace(
                "gb28181: ps map audio es_type=%s(%x), es_id=%0x, "
                "es_info_length=%d",
          ...
          if (video_es_type != type) {
            srs_trace(
                "gb28181: ps map video es_type=%s(%x), es_id=%0x, "
                "es_info_length=%d",
                s_type.c_str(), type, es_id, es_info_length);
          }
      ...
      if (pse_index == 0 && pts_dts_flags > 0) {
        ...
        srs_info("gb28181: ps stream video ts=%u pkt_ts=%u", video_pts,
                 timestamp);
      }
      pse_index += 1;
      ...
      complete_len =
          complete_len + (payload_len + 9 + pse_pack->stuffing_length);
      incomplete_len = ps_size - complete_len;
    } else if (next_ps_pack && next_ps_pack[0] == (char)0x00 &&
               next_ps_pack[1] == (char)0x00 && next_ps_pack[2] == (char)0x01 &&
      ...
      if (pts_dts_flags > 0) {
        ...
        srs_info("gb28181: ps stream video ts=%u pkt_ts=%u", audio_pts,
                 timestamp);
      }
      ...
      if (audio_enable && audio_es_type != STREAM_TYPE_AUDIO_AAC &&
        ...
        srs_error_t err2 = srs_success;
        if ((err2 = aac->adts_demux(&avs, &frame, &frame_size, codec)) !=
            srs_success) {
          srs_info(
              "gb28181: client_id %s, audio data not aac adts (%#x/%u) %02x "
              "%02x\n",
              channel_id.c_str(), ssrc, timestamp, p1, p2);
          srs_error_reset(err);
        } else {
          srs_warn(
              "gb28181: client_id %s, ps map is not aac (%s) type, but stream "
              "many be aac adts, try update type",
      ...
      if (hander && audio_enable && audio_stream.length() &&
          can_send_ps_av_packet()) {
        if ((err = hander->on_rtp_audio(&audio_stream, audio_pts,
                                        audio_es_type)) != srs_success) {
      ...
      if (next_ps_pack && next_ps_pack[0] == (char)0x00 &&
          next_ps_pack[1] == (char)0x00 && next_ps_pack[2] == (char)0x00 &&
          next_ps_pack[3] == (char)0x01) {
        // dahua's PS header may lose packets. It is sent by an RTP packet of
        // Dahua's PS header dahua rtp send format: ts=1000 seq=1 mark=false
        // payload= ps header ts=1000 seq=2 mark=false payload= video ts=1000
        // seq=3 mark=true payload= video ts=1000 seq=4 mark=true payload= audio
        incomplete_len = ps_size - complete_len;
      ...
      first_keyframe_flag = false;
      srs_trace(
          "gb28181: client_id %s, unkonw ps data (%#x/%u) %02x %02x %02x "
          "%02x\n",
          channel_id.c_str(), ssrc, timestamp, next_ps_pack[0] & 0xFF,
          next_ps_pack[1] & 0xFF, next_ps_pack[2] & 0xFF,
          next_ps_pack[3] & 0xFF);
      break;
    }
  }
...
SrsGb28181Config::SrsGb28181Config(SrsConfDirective* c) {
  ...
  auto_create_channel =
      _srs_config->get_stream_caster_gb28181_auto_create_channel(c);
  jitterbuffer_enable =
      _srs_config->get_stream_caster_gb28181_jitterbuffer_enable(c);
  ...
  sip_keepalive_timeout =
      _srs_config->get_stream_caster_gb28181_keepalive_timeout(c);
  sip_invite_port_fixed =
      _srs_config->get_stream_caster_gb28181_sip_invite_port_fixed(c);
  sip_query_catalog_interval =
      _srs_config->get_stream_caster_gb28181_sip_query_catalog_interval(c);
  sip_register_authentication =
      _srs_config->get_stream_caster_gb28181_sip_register_authentication(c);
  sip_register_password =
      _srs_config->get_stream_caster_gb28181_sip_register_password(c);
}

SrsGb28181Config::~SrsGb28181Config() {}

// SrsGb28181RtmpMuxer gb28181 rtmp muxer, process ps stream to rtmp
SrsGb28181RtmpMuxer::SrsGb28181RtmpMuxer(SrsGb28181Manger* c, std::string id,
                                         bool a, bool k) {
  channel_id = id;
  gb28181_manger = c;
...
std::string SrsGb28181RtmpMuxer::remote_ip() { return ""; }

const SrsContextId& SrsGb28181RtmpMuxer::get_id() {
  return _srs_context->get_id();
}

std::string SrsGb28181RtmpMuxer::desc() { return "GBConn"; }

std::string SrsGb28181RtmpMuxer::get_channel_id() { return channel_id; }

void SrsGb28181RtmpMuxer::copy_channel(SrsGb28181StreamChannel* s) {
  channel->copy(s);
}

SrsGb28181StreamChannel SrsGb28181RtmpMuxer::get_channel() { return *channel; }
...
void SrsGb28181RtmpMuxer::set_rtmp_url(std::string url) { _rtmp_url = url; }
std::string SrsGb28181RtmpMuxer::rtmp_url() { return _rtmp_url; }
...
srs_error_t SrsGb28181RtmpMuxer::initialize(SrsServer* s, SrsRequest* r) {
  ...
  if ((err = _srs_sources->fetch_or_create(req, (ISrsLiveSourceHandler*)server,
                                           &source)) != srs_success) {
  ...
  if (rtc_server_enabled && rtc_enabled) {
    ...
    if (!rtc->can_publish()) {
      return srs_error_new(ERROR_RTC_SOURCE_BUSY, "gb28181 rtc stream %s busy",
  ...
  while (true) {
    ...
    if (config.jitterbuffer_enable) {
      if (jitter_buffer->FoundFrame(cur_timestamp)) {
        jitter_buffer->GetFrame(&ps_buffer, ps_buflen, buffer_size, keyframe,
                                cur_timestamp);

        if (buffer_size > 0) {
          if ((err = ps_demixer->on_ps_stream(
                   ps_buffer, buffer_size, cur_timestamp, 0)) != srs_success) {
            srs_warn("gb28181: demix ps stream error:%s",
      ...
      if (jitter_buffer_audio->FoundFrame(cur_timestamp)) {
        jitter_buffer_audio->GetFrame(&ps_buffer_audio, ps_buflen_auido,
                                      buffer_size, keyframe, cur_timestamp);

        if (buffer_size > 0) {
          if ((err = ps_demixer->on_ps_stream(ps_buffer_audio, buffer_size,
                                              cur_timestamp, 0)) !=
              srs_success) {
            srs_warn("gb28181: demix ps stream error:%s",
                     srs_error_desc(err).c_str());
      ...
      while (!ps_queue.empty()) {
        ...
        if (pkt) {
          if ((err = ps_demixer->on_ps_stream(
                   pkt->payload->bytes(), pkt->payload->length(),
                   pkt->timestamp, pkt->ssrc)) != srs_success) {
            srs_warn("gb28181: demix ps stream error:%s",
                     srs_error_desc(err).c_str());
            srs_freep(err);
          };
    ...
    if (pprint->can_print()) {
      srs_trace(
          "gb28181: client id=%s,  ssrc=%#x, peer(%s, %d), rtmp muxer is alive",
          channel_id.c_str(), channel->get_ssrc(),
          channel->get_rtp_peer_ip().c_str(), channel->get_rtp_peer_port());
    }
    ...
    if (duration > (2 * SRS_UTIME_SECONDS) &&
        channel->get_rtp_peer_port() != 0) {
      srs_warn(
          "gb28181: client id=%s ssrc=%#x, peer(%s, %d), no rtp data %d in "
          "seconds, clean it, wait other port!",
          channel_id.c_str(), channel->get_ssrc(),
          channel->get_rtp_peer_ip().c_str(), channel->get_rtp_peer_port(),
          duration / SRS_UTIME_SECONDS);
      channel->set_rtp_peer_port(0);
      channel->set_rtp_peer_ip("");
    }

    if (duration > config.rtp_idle_timeout) {
      srs_trace("gb28181 log: duration = %ld, rtp idle timeout = %ld", duration,
                config.rtp_idle_timeout);
      srs_trace("gb28181: client id=%s, stream idle timeout, stop!!!",
                channel_id.c_str());
      break;
    }
    ...
    if (will_timeout && sdk) {
      srs_warn(
          "gb28181: client id=%s RTMP connection is about to time out without "
          "receiving any data",
          channel_id.c_str());
      rtmp_close();
    }
...
void SrsGb28181RtmpMuxer::insert_jitterbuffer(SrsPsRtpPacket* pkt) {
  ...
  if (av_same_ts) {
    pkt->marker = false;
    jitter_buffer->InsertPacket(pkt->sequence_number, pkt->timestamp,
                                pkt->marker, pkt->payload->bytes(),
                                pkt->payload->length(), NULL);
    ps_rtp_video_ts = pkt->timestamp;
  } else {
    jitter_buffer_audio->InsertPacket(pkt->sequence_number, pkt->timestamp,
                                      pkt->marker, pkt->payload->bytes(),
                                      pkt->payload->length(), NULL);
  }
...
srs_error_t SrsGb28181RtmpMuxer::cycle() {
  ...
  if (err == srs_success) {
    srs_trace("gb28181: client finished.");
  } else if (srs_is_client_gracefully_close(err)) {
    srs_warn("gb28181: client disconnect peer. code=%d", srs_error_code(err));
    srs_freep(err);
  }
...
srs_error_t SrsGb28181RtmpMuxer::on_rtp_video(SrsSimpleStream* stream,
                                              int64_t fpts) {
  srs_error_t err = srs_success;
...
srs_error_t SrsGb28181RtmpMuxer::write_h264_ipb_frame2(char* frame,
                                                       int frame_size,
                                                       uint32_t pts,
                                                       uint32_t dts) {
  srs_error_t err = srs_success;
  ...
  if ((err = write_h264_ipb_frame(frame, frame_size, dts, pts)) !=
      srs_success) {
...
srs_error_t SrsGb28181RtmpMuxer::replace_startcode_with_nalulen(
    char* video_data, int& size, uint32_t pts, uint32_t dts) {
  srs_error_t err = srs_success;
...
srs_error_t SrsGb28181RtmpMuxer::on_rtp_audio(SrsSimpleStream* stream,
                                              int64_t fdts, int type) {
  srs_error_t err = srs_success;
  ...
  if (!avs->empty()) {
    if (type == STREAM_TYPE_AUDIO_AAC) {
      ...
      int frame_size = 0;
      SrsRawAacStreamCodec codec;
      if ((err = aac->adts_demux(avs, &frame, &frame_size, codec)) !=
          srs_success) {
      ...
      static int srs_aac_srates[] = {96000, 88200, 64000, 48000, 44100, 32000,
      ...
      if (aac_specific_config != sh) {
        ...
        if ((err = write_audio_raw_frame((char*)sh.data(), (int)sh.length(),
                                         &codec, dts)) != srs_success) {
      ...
      if (send_adts) {  // audio raw data. with  adts header
        if ((err = write_audio_raw_frame(stream->bytes(), stream->length(),
                                         &codec, dts)) != srs_success) {
          return srs_error_wrap(err, "write audio raw frame");
        }
      } else {  // audio raw data. without  adts header
        if ((err = write_audio_raw_frame(frame, frame_size, &codec, dts)) !=
            srs_success) {
          return srs_error_wrap(err, "write audio raw frame");
        }
      }
      ...
      if (type == STREAM_TYPE_AUDIO_G711) {
        ...
        return srs_error_wrap(
            err, "write audio raw frame, type=%d not suppered", type);
      }
...
srs_error_t SrsGb28181RtmpMuxer::write_h264_sps_pps(uint32_t dts,
                                                    uint32_t pts) {
  srs_error_t err = srs_success;
  ...
  std::string sh;
  if ((err = avc->mux_sequence_header(h264_sps, h264_pps, dts, pts, sh)) !=
      srs_success) {
    return srs_error_wrap(err, "mux sequence header");
  }
  ...
  char* flv = NULL;
  int nb_flv = 0;
  if ((err = avc->mux_avc2flv(sh, frame_type, avc_packet_type, dts, pts, &flv,
                              &nb_flv)) != srs_success) {
  ...
  if ((err = rtmp_write_packet(SrsFrameTypeVideo, timestamp, flv, nb_flv)) !=
      srs_success) {
...
srs_error_t SrsGb28181RtmpMuxer::write_h264_ipb_frame(
    char* frame, int frame_size, uint32_t dts, uint32_t pts, bool writelen) {
  srs_error_t err = srs_success;
  ...
  char* flv = NULL;
  int nb_flv = 0;
  if ((err = avc->mux_avc2flv(ibp, frame_type, avc_packet_type, dts, pts, &flv,
                              &nb_flv)) != srs_success) {
    return srs_error_wrap(err, "mux avc to flv");
  }
...
srs_error_t SrsGb28181RtmpMuxer::write_audio_raw_frame(
    char* frame, int frame_size, SrsRawAacStreamCodec* codec, uint32_t dts) {
  srs_error_t err = srs_success;

  char* data = NULL;
  int size = 0;
  if ((err = aac->mux_aac2flv(frame, frame_size, codec, dts, &data, &size)) !=
      srs_success) {
...
srs_error_t SrsGb28181RtmpMuxer::rtmp_write_packet(char type,
                                                   uint32_t timestamp,
                                                   char* data, int size) {
  srs_error_t err = srs_success;
  ...
  if ((err = srs_rtmp_create_msg(type, timestamp, data, size, sdk->sid(),
                                 &msg)) != srs_success) {
...
srs_error_t SrsGb28181RtmpMuxer::rtmp_write_packet_by_source(char type,
                                                             uint32_t timestamp,
                                                             char* data,
                                                             int size) {
  srs_error_t err = srs_success;
  ...
  // create a source that will process stream without the need for internal
  // rtmpclient
...
srs_error_t SrsGb28181RtmpMuxer::connect() {
  ...
  if ((err = sdk->connect()) != srs_success) {
    ...
    return srs_error_wrap(err, "connect %s failed, cto=%dms, sto=%dms.",
                          url.c_str(), srsu2msi(cto), srsu2msi(sto));
  }

  // publish.
  if ((err = sdk->publish(SRS_CONSTS_RTMP_PROTOCOL_CHUNK_SIZE)) !=
      srs_success) {
    close();
...
void SrsGb28181RtmpMuxer::rtmp_close() { close(); }
...
SrsGb28181Config SrsGb28181Manger::get_gb28181_config() { return *config; }
...
srs_error_t SrsGb28181Manger::fetch_or_create_rtmpmuxer(
    std::string id, SrsRequest* req, SrsGb28181RtmpMuxer** gb28181) {
  srs_error_t err = srs_success;
  ...
  muxer = new SrsGb28181RtmpMuxer(this, id, config->audio_enable,
                                  config->wait_keyframe);
...
void SrsGb28181Manger::update_rtmpmuxer_to_newssrc_by_id(std::string id,
                                                         uint32_t ssrc) {
  SrsGb28181RtmpMuxer* muxer = NULL;

  if (rtmpmuxers.find(id) == rtmpmuxers.end()) {
    srs_warn(
        "gb28181: at update_rtmpmuxer_to_newssrc_by_id() client_id not found. "
        "client_id=%s",
        id.c_str());
    return;
  }
  ...
  if (old_ssrc == ssrc) {
    ...
    srs_trace("gb28181: update ssrc of muxer %s from %x to %x", id.c_str(),
              old_ssrc, ssrc);
  }
...
void SrsGb28181Manger::rtmpmuxer_map_by_ssrc(SrsGb28181RtmpMuxer* muxer,
                                             uint32_t ssrc) {
  if (rtmpmuxers_ssrc.find(ssrc) == rtmpmuxers_ssrc.end()) {
...
void SrsGb28181Manger::rtmpmuxer_unmap_by_ssrc(uint32_t ssrc) {
  std::map<uint32_t, SrsGb28181RtmpMuxer*>::iterator it =
      rtmpmuxers_ssrc.find(ssrc);
...
srs_error_t SrsGb28181Manger::start_ps_rtp_listen(std::string id, int port) {
  ...
  /* delete by xbpeng 20201222 should not check rtmpmuxers, becasue it always
   * not find*/
  // map<std::string, SrsGb28181RtmpMuxer*>::iterator key = rtmpmuxers.find(id);
  // if (key == rtmpmuxers.end()){
  //     srs_warn("start rtp listen port rtmp muxer is null. id=%s,port=%d",
  //     id.c_str(),port); return srs_error_wrap(err, "start rtp listen port
  //     rtmp muxer is null");
  // }
...
// api
srs_error_t SrsGb28181Manger::create_stream_channel(
    SrsGb28181StreamChannel* channel) {
  srs_error_t err = srs_success;
  srs_assert(channel);
  ...
  if (port_mode == RTP_PORT_MODE_RANDOM) {
    alloc_port(&rtp_port);
    if (rtp_port <= 0) {
      return srs_error_new(ERROR_GB28181_RTP_PORT_FULL,
                           "gb28181: rtp port full");
    }
  ...
  if (true) {
    ...
    srs_discovery_tc_url(tcUrl, _schema, _host, _vhost, _app, _stream,
                         rtmp_port, _param);
    ...
    std::string play_url = srs_generate_rtmp_url(config->host, rtmp_port, "",
                                                 "", app, stream_name, "");

    std::string flv_url = srs_string_replace(play_url, "rtmp://", "http://");
    std::stringstream port;
    port << ":" << rtmp_port;
    flv_url = srs_string_replace(flv_url, port.str(),
                                 ":" + _srs_config->get_http_stream_listen());
    std::string hls_url = flv_url + ".m3u8";
    flv_url = flv_url + ".flv";

    std::string webrtc_url =
        srs_string_replace(play_url, "rtmp://", "webrtc://");
    webrtc_url = srs_string_replace(webrtc_url, port.str(),
                                    ":" + _srs_config->get_http_api_listen());
  ...
  if ((err = fetch_or_create_rtmpmuxer(id, &request, &muxer)) != srs_success) {
    srs_warn("gb28181: create rtmp muxer error, %s",
             srs_error_desc(err).c_str());
    return err;
  }
  ...
  srs_trace("gb28181: create new stream channel id:%s rtmp url=%s", id.c_str(),
            url.c_str());
...
srs_error_t SrsGb28181Manger::delete_stream_channel(std::string id,
  ...
  if (muxer) {
    ...
    return srs_error_new(ERROR_GB28181_SESSION_IS_NOTEXIST,
                         "stream channel is not exists");
  }
}

srs_error_t SrsGb28181Manger::query_stream_channel(std::string id,
                                                   SrsJsonArray* arr) {
  srs_error_t err = srs_success;
...
srs_error_t SrsGb28181Manger::notify_sip_invite(std::string id, std::string ip,
  ...
  if (!muxer) {
    // if there is an invalid parameter, the channel will be created
    // automatically
...
srs_error_t SrsGb28181Manger::notify_sip_ptz(std::string id, std::string chid,
...
srs_error_t SrsGb28181Manger::notify_sip_raw_data(std::string id,
                                                  std::string data) {
  if (!sip_service) {
    return srs_error_new(ERROR_GB28181_SIP_NOT_RUN, "sip not run");
  }
...
srs_error_t SrsGb28181Manger::notify_sip_query_catalog(std::string id) {
  ...
  if (sip_session) {
    ...
    srs_trace(
        "notify_sip_query_catalog, clear sip session item and device list");
  }
  return sip_service->send_query_catalog(&req);
}

srs_error_t SrsGb28181Manger::query_sip_session(std::string id,
                                                SrsJsonArray* arr) {
  if (!sip_service) {
    return srs_error_new(ERROR_GB28181_SIP_NOT_RUN, "sip not run");
  }
...
srs_error_t SrsGb28181Manger::query_device_list(std::string id,
                                                SrsJsonArray* arr) {
  if (!sip_service) {
    return srs_error_new(ERROR_GB28181_SIP_NOT_RUN, "sip not run");
  }
...
SrsGb28181Conn::SrsGb28181Conn(SrsGb28181Caster* c, srs_netfd_t fd,
...
srs_error_t SrsGb28181Conn::do_cycle() {
  ...
  while (true) {
    ...
    nb_read = 0;
    if ((err = skt->read(mbuffer + left_data_len,
                         SRS_RTSP_BUFFER - left_data_len, &nb_read)) !=
        srs_success) {
    ...
    for (; index < left_data_len;) {
      ...
      if (packet_len > MAX_PACKAGE_SIZE) {
        // FIXME 自动重新invite?
        srs_error("abnormal RTP packet length:%d, close the tcp conn:%s",
                  packet_len, remote_ip().c_str());
        return err;
      }
      if (index + RTP_TCP_HEADER + packet_len >= left_data_len) {
        break;
      }
      processor->on_tcp_packet((sockaddr*)peer_sockaddr, addr_len,
                               buf + index + RTP_TCP_HEADER, packet_len);
...
std::string SrsGb28181Conn::desc() { return "GB28181TcpConn"; }

const SrsContextId& SrsGb28181Conn::get_id() { return trd->cid(); }
...
void SrsGb28181Caster::remove(SrsGb28181Conn* conn) {
  std::vector<SrsGb28181Conn*>::iterator it =
      find(clients.begin(), clients.end(), conn);
  if (it != clients.end()) {
    clients.erase(it);
  }
