...
SrsSimpleRtmpClient::SrsSimpleRtmpClient(string u, srs_utime_t ctm,
                                         srs_utime_t stm)
    : SrsBasicRtmpClient(u, ctm, stm) {}
...
std::string SrsRtmpConn::desc() { return "RtmpConn"; }

// TODO: return detail message when error for client.
srs_error_t SrsRtmpConn::do_cycle() {
  srs_error_t err = srs_success;

  srs_trace("RTMP client ip=%s:%d, fd=%d", ip.c_str(), port,
            srs_netfd_fileno(stfd));
  ...
  if (rip > 0) {
    srs_trace("RTMP proxy real client ip=%d.%d.%d.%d", uint8_t(rip >> 24),
              uint8_t(rip >> 16), uint8_t(rip >> 8), uint8_t(rip));
  }
  ...
  srs_trace(
      "connect app, tcUrl=%s, pageUrl=%s, swfUrl=%s, schema=%s, vhost=%s, "
      "port=%d, app=%s, args=%s",
...
srs_error_t SrsRtmpConn::on_reload_vhost_removed(string vhost) {
  ...
  srs_trace("vhost %s removed/disabled, close client url=%s", vhost.c_str(),
...
srs_error_t SrsRtmpConn::on_reload_vhost_play(string vhost) {
  ...
  if (true) {
    ...
    if (v != send_min_interval) {
      srs_trace("apply smi %d=>%d ms", srsu2msi(send_min_interval),
                srsu2msi(v));
      send_min_interval = v;
    }
  }
...
srs_error_t SrsRtmpConn::on_reload_vhost_publish(string vhost) {
  ...
  if (p1stpt != publish_1stpkt_timeout) {
    srs_trace("p1stpt changed %d=>%d", srsu2msi(publish_1stpkt_timeout),
              srsu2msi(p1stpt));
  ...
  if (pnt != publish_normal_timeout) {
    srs_trace("pnt changed %d=>%d", srsu2msi(publish_normal_timeout),
              srsu2msi(pnt));
...
void SrsRtmpConn::remark(int64_t* in, int64_t* out) { kbps->remark(in, out); }
...
srs_error_t SrsRtmpConn::service_cycle() {
  ...
  if (_srs_config->get_bw_check_enabled(req->vhost)) {
    if ((err = bandwidth->bandwidth_check(rtmp, skt, req, local_ip)) !=
        srs_success) {
  ...
  if ((err = rtmp->response_connect_app(req, local_ip.c_str())) !=
      srs_success) {
  ...
  while (true) {
    ...
    // TODO: FIXME: Support RTMP client timeout,
    // https://github.com/ossrs/srs/issues/1134
    if (err == srs_success) {
      continue;
    }
...
srs_error_t SrsRtmpConn::stream_service_cycle() {
  ...
  if ((err = rtmp->identify_client(info->res->stream_id, info->type,
                                   req->stream, req->duration)) !=
      srs_success) {
    return srs_error_wrap(err, "rtmp: identify client");
  }

  srs_discovery_tc_url(req->tcUrl, req->schema, req->host, req->vhost, req->app,
                       req->stream, req->port, req->param);
  req->strip();
  srs_trace(
      "client identified, type=%s, vhost=%s, app=%s, stream=%s, param=%s, "
      "duration=%dms",
      srs_client_type_string(info->type).c_str(), req->vhost.c_str(),
      req->app.c_str(), req->stream.c_str(), req->param.c_str(),
      srsu2msi(req->duration));
  ...
  if (req->schema.empty() || req->vhost.empty() || req->port == 0 ||
      req->app.empty()) {
    return srs_error_new(ERROR_RTMP_REQ_TCURL,
                         "discovery tcUrl failed, tcUrl=%s, schema=%s, "
                         "vhost=%s, port=%d, app=%s",
                         req->tcUrl.c_str(), req->schema.c_str(),
                         req->vhost.c_str(), req->port, req->app.c_str());
  }
  ...
  srs_trace(
      "connected stream, tcUrl=%s, pageUrl=%s, swfUrl=%s, schema=%s, vhost=%s, "
      "port=%d, app=%s, stream=%s, param=%s, args=%s",
      req->tcUrl.c_str(), req->pageUrl.c_str(), req->swfUrl.c_str(),
      req->schema.c_str(), req->vhost.c_str(), req->port, req->app.c_str(),
      req->stream.c_str(), req->param.c_str(), (req->args ? "(obj)" : "null"));
  ...
  // Never allow the empty stream name, for HLS may write to a file with empty
  // name.
  ...
  if ((err = _srs_sources->fetch_or_create(req, server, &source)) !=
      srs_success) {
  ...
  if ((err = stat->on_client(_srs_context->get_id().c_str(), req, this,
                             info->type)) != srs_success) {
    return srs_error_wrap(err, "rtmp: stat client");
  }

  bool enabled_cache = _srs_config->get_gop_cache(req->vhost);
  srs_trace("source url=%s, ip=%s, cache=%d, is_edge=%d, source_id=%s/%s",
            req->get_stream_url().c_str(), ip.c_str(), enabled_cache,
            info->edge, source->source_id().c_str(),
  ...
  switch (info->type) {
    ...
    case SrsRtmpConnFMLEPublish: {
      if ((err = rtmp->start_fmle_publish(info->res->stream_id)) !=
          srs_success) {
    ...
    case SrsRtmpConnHaivisionPublish: {
      if ((err = rtmp->start_haivision_publish(info->res->stream_id)) !=
          srs_success) {
    ...
    case SrsRtmpConnFlashPublish: {
      if ((err = rtmp->start_flash_publish(info->res->stream_id)) !=
          srs_success) {
    ...
    default: {
      return srs_error_new(ERROR_SYSTEM_CLIENT_INVALID,
                           "rtmp: unknown client type=%d", info->type);
    }
  }
...
srs_error_t SrsRtmpConn::check_vhost(bool try_default_vhost) {
  ...
  SrsConfDirective* vhost =
      _srs_config->get_vhost(req->vhost, try_default_vhost);
  if (vhost == NULL) {
    return srs_error_new(ERROR_RTMP_VHOST_NOT_FOUND, "rtmp: no vhost %s",
                         req->vhost.c_str());
  }

  if (!_srs_config->get_vhost_enabled(req->vhost)) {
    return srs_error_new(ERROR_RTMP_VHOST_NOT_FOUND, "rtmp: vhost %s disabled",
                         req->vhost.c_str());
  }

  if (req->vhost != vhost->arg0()) {
    srs_trace("vhost change from %s to %s", req->vhost.c_str(),
              vhost->arg0().c_str());
    req->vhost = vhost->arg0();
  }

  if (_srs_config->get_refer_enabled(req->vhost)) {
    if ((err = refer->check(req->pageUrl, _srs_config->get_refer_all(
                                              req->vhost))) != srs_success) {
...
srs_error_t SrsRtmpConn::playing(SrsLiveSource* source) {
  ...
  SrsRequest* req = info->req;
  if (_srs_config->get_refer_enabled(req->vhost)) {
    if ((err = refer->check(req->pageUrl, _srs_config->get_refer_play(
                                              req->vhost))) != srs_success) {
      return srs_error_wrap(err, "rtmp: referer check");
    }
  }
  ...
  if (!info->edge && _srs_config->get_vhost_origin_cluster(req->vhost) &&
      source->inactive()) {
    vector<string> coworkers = _srs_config->get_vhost_coworkers(req->vhost);
    for (int i = 0; i < (int)coworkers.size(); i++) {
      // TODO: FIXME: User may config the server itself as coworker, we must
      ...
      string url = "http://" + coworker + "/api/v1/clusters?" +
                   "vhost=" + req->vhost + "&ip=" + req->host +
                   "&app=" + req->app + "&stream=" + req->stream +
                   "&coworker=" + coworker;
      if ((err = SrsHttpHooks::discover_co_workers(url, host, port)) !=
          srs_success) {
        // If failed to discovery stream in this coworker, we should request the
        // next one util the last.
      ...
      srs_trace(
          "rtmp: redirect in cluster, from=%s:%d, target=%s:%d, url=%s, "
          "rurl=%s",
...
srs_error_t SrsRtmpConn::do_playing(SrsLiveSource* source,
  ...
  srs_trace(
      "start play smi=%dms, mw_sleep=%d, mw_msgs=%d, realtime=%d, "
      "tcp_nodelay=%d",
      srsu2msi(send_min_interval), srsu2msi(mw_sleep), mw_msgs, realtime,
      tcp_nodelay);
  ...
  while (true) {
    ...
    // each msg in msgs.msgs must be free, for the SrsMessageArray never free
    // them.
    ...
    if (pprint->can_print()) {
      ...
      srs_trace("-> " SRS_CONSTS_LOG_PLAY
                " time=%d, msgs=%d, okbps=%d,%d,%d, ikbps=%d,%d,%d, mw=%d/%d",
                (int)pprint->age(), count, kbps->get_send_kbps(),
                kbps->get_send_kbps_30s(), kbps->get_send_kbps_5m(),
                kbps->get_recv_kbps(), kbps->get_recv_kbps_30s(),
                kbps->get_recv_kbps_5m(), srsu2msi(mw_sleep), mw_msgs);
    }
    ...
    if (user_specified_duration_to_stop) {
      for (int i = 0; i < count; i++) {
        ...
        // @remark: never use msg when sent it, for the protocol sdk will free
        // it.
    ...
    if (count > 0 &&
        (err = rtmp->send_and_free_messages(
             msgs.msgs, count, info->res->stream_id)) != srs_success) {
    ...
    if (user_specified_duration_to_stop) {
      if (duration >= req->duration) {
        return srs_error_new(ERROR_RTMP_DURATION_EXCEED, "rtmp: time %d up %d",
                             srsu2msi(duration), srsu2msi(req->duration));
      }
    }
...
srs_error_t SrsRtmpConn::publishing(SrsLiveSource* source) {
  ...
  if (_srs_config->get_refer_enabled(req->vhost)) {
    if ((err = refer->check(req->pageUrl, _srs_config->get_refer_publish(
                                              req->vhost))) != srs_success) {
      return srs_error_wrap(err, "rtmp: referer check");
    }
  }
  ...
  if ((err = acquire_publish(source)) == srs_success) {
    ...
    SrsPublishRecvThread rtrd(rtmp, req, srs_netfd_fileno(stfd), 0, this,
                              source, _srs_context->get_id());
...
srs_error_t SrsRtmpConn::do_publishing(SrsLiveSource* source,
  ...
  if (true) {
    ...
    srs_trace("start publish mr=%d/%d, p1stpt=%d, pnt=%d, tcp_nodelay=%d", mr,
              srsu2msi(mr_sleep), srsu2msi(publish_1stpkt_timeout),
              srsu2msi(publish_normal_timeout), tcp_nodelay);
  }
  ...
  while (true) {
    ...
    if (rtrd->nb_msgs() <= nb_msgs) {
      return srs_error_new(ERROR_SOCKET_TIMEOUT,
                           "rtmp: publish timeout %dms, nb_msgs=%d",
                           nb_msgs ? srsu2msi(publish_normal_timeout)
                                   : srsu2msi(publish_1stpkt_timeout),
                           (int)nb_msgs);
    }
    ...
    SrsStatistic* stat = SrsStatistic::instance();
    if ((err = stat->on_video_frames(
             req, (int)(rtrd->nb_video_frames() - nb_frames))) != srs_success) {
    ...
    if (pprint->can_print()) {
      ...
      srs_trace("<- " SRS_CONSTS_LOG_CLIENT_PUBLISH
                " time=%d, okbps=%d,%d,%d, ikbps=%d,%d,%d, mr=%d/%d, "
                "p1stpt=%d, pnt=%d",
                (int)pprint->age(), kbps->get_send_kbps(),
                kbps->get_send_kbps_30s(), kbps->get_send_kbps_5m(),
                kbps->get_recv_kbps(), kbps->get_recv_kbps_30s(),
                kbps->get_recv_kbps_5m(), mr, srsu2msi(mr_sleep),
                srsu2msi(publish_1stpkt_timeout),
                srsu2msi(publish_normal_timeout));
    }
  }
...
srs_error_t SrsRtmpConn::acquire_publish(SrsLiveSource* source) {
  ...
  if (_srs_gb28181 != NULL) {
    ...
    if (gb28181 != NULL) {
      return srs_error_new(ERROR_SYSTEM_STREAM_BUSY, "gb28181 stream %s busy",
                           req->get_stream_url().c_str());
    }
  }
#endif
  ...
  if (!source->can_publish(info->edge)) {
    return srs_error_new(ERROR_SYSTEM_STREAM_BUSY, "rtmp: stream %s is busy",
                         req->get_stream_url().c_str());
  }
  ...
  if (rtc_server_enabled && rtc_enabled && !info->edge) {
    ...
    if (!rtc->can_publish()) {
      return srs_error_new(ERROR_SYSTEM_STREAM_BUSY, "rtc stream %s busy",
                           req->get_stream_url().c_str());
    }
  }
#endif
...
srs_error_t SrsRtmpConn::handle_publish_message(SrsLiveSource* source,
  ...
  if (msg->header.is_amf0_command() || msg->header.is_amf3_command()) {
    ...
    if (dynamic_cast<SrsFMLEStartPacket*>(pkt)) {
      ...
      if ((err = rtmp->fmle_unpublish(info->res->stream_id,
                                      unpublish->transaction_id)) !=
          srs_success) {
...
srs_error_t SrsRtmpConn::process_play_control_msg(SrsLiveConsumer* consumer,
                                                  SrsCommonMessage* msg) {
  srs_error_t err = srs_success;
  ...
  if (pause) {
    if ((err = rtmp->on_play_client_pause(info->res->stream_id,
                                          pause->is_pause)) != srs_success) {
      return srs_error_wrap(err, "rtmp: pause");
    }
    if ((err = consumer->on_play_client_pause(pause->is_pause)) !=
        srs_success) {
      return srs_error_wrap(err, "rtmp: pause");
    }
    return err;
...
srs_error_t SrsRtmpConn::check_edge_token_traverse_auth() {
  ...
  for (int i = 0; i < (int)args.size(); i++) {
    ...
    SrsTcpClient* transport =
        new SrsTcpClient(server, port, SRS_EDGE_TOKEN_TRAVERSE_TIMEOUT);
    SrsAutoFree(SrsTcpClient, transport);

    if ((err = transport->connect()) != srs_success) {
      srs_warn("Illegal edge token, tcUrl=%s, %s", req->tcUrl.c_str(),
  ...
  return srs_error_new(ERROR_EDGE_PORT_INVALID,
                       "rtmp: Illegal edge token, server=%d", (int)args.size());
}
...
srs_error_t SrsRtmpConn::do_token_traverse_auth(SrsRtmpClient* client) {
  ...
  SrsServerInfo si;
  if ((err = client->connect_app(req->app, req->tcUrl, req, true, &si)) !=
      srs_success) {
    return srs_error_wrap(err, "rtmp: connect tcUrl");
  }
...
void SrsRtmpConn::http_hooks_on_close() {
  ...
  for (int i = 0; i < (int)hooks.size(); i++) {
    std::string url = hooks.at(i);
    SrsHttpHooks::on_close(url, req, kbps->get_send_bytes(),
                           kbps->get_recv_bytes());
  }
}
...
srs_error_t SrsRtmpConn::cycle() {
  ...
  // success.
  if (err == srs_success) {
    srs_trace("rtmp: client finished.");
    return err;
  }

  // It maybe success with message.
  if (srs_error_code(err) == ERROR_SUCCESS) {
    srs_trace("rtmp: client finished%s.", srs_error_summary(err).c_str());
    srs_freep(err);
    return err;
  }
  ...
  if (srs_is_client_gracefully_close(err)) {
    srs_warn("rtmp: client disconnect peer. ret=%d", srs_error_code(err));
  } else if (srs_is_server_gracefully_close(err)) {
    srs_warn("rtmp: server disconnect. ret=%d", srs_error_code(err));
  } else {
    srs_error("rtmp: serve error %s", srs_error_desc(err).c_str());
  }
...
string SrsRtmpConn::remote_ip() { return ip; }

const SrsContextId& SrsRtmpConn::get_id() { return trd->cid(); }

void SrsRtmpConn::expire() { trd->interrupt(); }
