...
std::string srs_sip_get_param_by_split(const std::string &msg,
...
std::string srs_get_sip_session_status_str(
    SrsGb28181SipSessionStatusType status) {
  switch (status) {
...
SrsGb28181SipSession::SrsGb28181SipSession(SrsGb28181SipService *c,
                                           SrsSipRequest *r) {
  servcie = c;
  ...
  while (true) {
    ...
    if (_register_status == SrsGb28181SipSessionRegisterOk &&
      ...
      while (auto_play_list.size() > 0) {
        ...
        srs_trace("gb28181: send invite = %s", chid.c_str());
        ...
        if ((err = _srs_gb28181->create_stream_channel(&ch)) == srs_success) {
          ...
          srs_trace("gb28181: send invite %s", ch.get_ip().c_str());
          err = servcie->send_invite(&req, ch.get_ip(), ch.get_rtp_port(),
                                     ch.get_ssrc(), chid);
        }
    ...
    if (_register_status == SrsGb28181SipSessionRegisterOk &&
      ...
      srs_trace("gb28181: sip session=%s keepalive timeout",
                _session_id.c_str());
      break;
    }
    ...
    if (_alive_status == SrsGb28181SipSessionAliveOk &&
      ...
      if (err != srs_success) {
        srs_trace("gb28181: sip query catalog error %s",
      ...
      // print device status
      srs_trace(
          "gb28181: sip session=%s peer(%s, %d) status(%s,%s) duration(%u,%u)",
      ...
      for (it = _device_list.begin(); it != _device_list.end(); it++) {
        ...
        srs_utime_t invite_duration =
            srs_get_system_time() - device->invite_time;
        ...
        srs_info(
            "gb28181: sip session=%s device=%s status(%s, %s), duration(%u)",
...
std::string SrsGb28181SipSession::remote_ip() { return _peer_ip; }

const SrsContextId &SrsGb28181SipSession::get_id() {
  return _srs_context->get_id();
}

std::string SrsGb28181SipSession::desc() { return "SipConn"; }
...
void SrsGb28181SipSession::update_device_list(
  ...
  for (it = _device_list.begin(); it != _device_list.end(); ++it) {
    ...
    obj->set(
        ...
        SrsJsonAny::str(
            srs_get_sip_session_status_str(device->invite_status).c_str()));
    obj->set("invite_time",
             SrsJsonAny::integer(device->invite_time / SRS_UTIME_SECONDS));
  }
}
...
SrsGb28181Config *SrsGb28181SipService::get_config() { return config; }

void SrsGb28181SipService::set_stfd(srs_netfd_t fd) { lfd = fd; }

srs_error_t SrsGb28181SipService::on_udp_packet(const sockaddr *from,
                                                const int fromlen, char *buf,
                                                int nb_buf) {
  char address_string[64];
  char port_string[16];
  if (getnameinfo(from, fromlen, (char *)&address_string,
                  sizeof(address_string), (char *)&port_string,
                  sizeof(port_string), NI_NUMERICHOST | NI_NUMERICSERV)) {
  ...
  srs_error_t err =
      on_udp_sip(peer_ip, peer_port, recv_msg, (sockaddr *)from, fromlen);
  if (err != srs_success) {
...
bool SrsGb28181SipService::register_authentication(
  ...
  */
  ...
  std::string responseStr = std::string(ha1) + ":" + nonce + ":" + nc + ":" +
                            cnonce + ":" + qop + ":" + std::string(ha2);

  md5::MD5 md53;
  char *responseRes = md53.digestString(const_cast<char *>(responseStr.data()));
...
srs_error_t SrsGb28181SipService::on_udp_sip(string peer_ip, int peer_port,
  ...
  srs_info("gb28181: request peer(%s, %d) nbbuf=%d", peer_ip.c_str(), peer_port,
           recv_len);
  ...
  if (req->is_register_authentication()) {
    srs_trace(
        "gb28181: register_authentication ---------------- sip_auth_id: %s\n",
        req->sip_auth_id.c_str());
    ...
    if (!sip_session ||
        sip_session->register_status() == SrsGb28181SipSessionUnkonw) {
      // srs_trace("gb28181: %s client not registered",
      // req->sip_auth_id.c_str());
      srs_trace("gb28181: %s client try to register authentication.",
                req->sip_auth_id.c_str());
    ...
    srs_trace(
        "gb28181: register_authentication is register ----------------\n");
    if (register_authentication(req->authorization)) {
      std::vector<std::string> serial =
          srs_string_split(srs_string_replace(req->uri, "sip:", ""), "@");
      if (serial.empty()) {
        return srs_error_new(ERROR_GB28181_SIP_PRASE_FAILED,
                             "register string split");
      }

      // if (serial.at(0) != config->sip_serial){
      //    srs_warn("gb28181: client:%s request serial and server serial
      //    inconformity(%s:%s)",
      //     req->sip_auth_id.c_str(), serial.at(0).c_str(),
      //     config->sip_serial.c_str());
      //    return  err;
      //}

      srs_trace("gb28181: request client id=%s peer(%s, %d)",
                req->sip_auth_id.c_str(), peer_ip.c_str(), peer_port);
      ...
      srs_trace("gb28181: client ip: %s register end.", peer_ip.c_str());
    } else {
      // std::cout << "gb28181: failed to register authentication." <<
      // std::endl;
      srs_trace("gb28181: failed to register authentication.");
    }
  } else if (req->is_message()) {
    srs_trace("gb28181: is message ----------------\n");
    SrsGb28181SipSession *sip_session = fetch(session_id);
    ...
    if (req->cmdtype == SrsSipCmdRequest) {
      ...
      if (!strcasecmp(req->content_type.c_str(), "application/manscdp+xml") &&
          req->xml_body_map.find("Response@CmdType") !=
              req->xml_body_map.end() &&
          req->xml_body_map["Response@CmdType"] == "Catalog") {
        if (req->xml_body_map.find("Response@SumNum") !=
            req->xml_body_map.end()) {
          sip_session->item_list_sumnum =
              atoi(req->xml_body_map["Response@SumNum"].c_str());
        }
        ...
        for (it = req->item_list.begin(); it != req->item_list.end(); ++it) {
          ...
          std::map<std::string, std::map<std::string, std::string> >::iterator
              it2 = sip_session->item_list.find(device["DeviceID"]);
    ...
    srs_trace("gb28181: %s method=%s, uri=%s, version=%s ",
              req->get_cmdtype_str().c_str(), req->method.c_str(),
              req->uri.c_str(), req->version.c_str());

    if (!sip_session) {
      srs_trace("gb28181: call_id %s not map %s client ", req->call_id.c_str(),
                req->sip_auth_id.c_str());
      return err;
    }
    ...
    if (sip_session->register_status() == SrsGb28181SipSessionUnkonw ||
      ...
      srs_trace("gb28181: %s client not registered or not alive",
                req->sip_auth_id.c_str());
      return err;
    }

    if (req->cmdtype == SrsSipCmdRespone) {
      srs_trace("gb28181: INVITE response %s client status=%s",
                req->sip_auth_id.c_str(), req->status.c_str());

      if (req->status == "200") {
        srs_trace("gb28181: device unique id is %s@%s",
                  sip_session->session_id().c_str(), req->sip_auth_id.c_str());
        // if srs is external realm, ssrc is generated by source realm rather
        // than srs so update ssrc to the y line in source realm '200 OK'
        // response actually, we should do this all the time
        if (req->y_ssrc != 0) {
          _srs_gb28181->update_rtmpmuxer_to_newssrc_by_id(
              sip_session->session_id() + "@" + req->sip_auth_id, req->y_ssrc);
          req->y_ssrc = 0;
        }
        send_ack(req, from, fromlen);
        SrsGb28181Device *device =
            sip_session->get_device_info(req->sip_auth_id);
        if (device) {
          device->invite_status = SrsGb28181SipSessionInviteOk;
    ...
    if (req->cmdtype == SrsSipCmdRespone) {
      srs_trace("gb28181: BYE  %s client status=%s", req->sip_auth_id.c_str(),
                req->status.c_str());
...
int SrsGb28181SipService::send_message(sockaddr *from, int fromlen,
                                       std::stringstream &ss) {
  std::string str = ss.str();
  srs_info("gb28181: send_message:%s", str.c_str());
  srs_assert(!str.empty());

  int ret = srs_sendto(lfd, (char *)str.c_str(), (int)str.length(), from,
                       fromlen, SRS_UTIME_NO_TIMEOUT);
  if (ret <= 0) {
...
srs_error_t SrsGb28181SipService::send_invite(SrsSipRequest *req, string ip,
  ...
  if (!device || device->device_status != "ON") {
    return srs_error_new(ERROR_GB28181_SIP_CH_OFFLINE,
                         "sip device channel offline");
  }

  if (device->invite_status == SrsGb28181SipSessionTrying ||
      device->invite_status == SrsGb28181SipSessionInviteOk) {
    return srs_error_new(ERROR_GB28181_SIP_IS_INVITING,
                         "sip device channel inviting");
  }
  ...
  if (send_message(&addr, sip_session->sockaddr_fromlen(), ss) <= 0) {
    return srs_error_new(ERROR_GB28181_SIP_INVITE_FAILED,
                         "sip device invite failed");
  }
...
srs_error_t SrsGb28181SipService::send_bye(SrsSipRequest *req,
                                           std::string chid) {
  srs_error_t err = srs_success;
  ...
  SrsGb28181Device *device = sip_session->get_device_info(chid);
  if (!device) {
    return srs_error_new(ERROR_GB28181_SIP_CH_NOTEXIST,
                         "sip device channel not exist");
  }
...
srs_error_t SrsGb28181SipService::send_sip_raw_data(SrsSipRequest *req,
                                                    std::string data) {
  srs_error_t err = srs_success;
  ...
  sockaddr addr = sip_session->sockaddr_from();
  if (send_message(&addr, sip_session->sockaddr_fromlen(), ss) <= 0) {
    return srs_error_new(ERROR_GB28181_SIP_RAW_DATA_FAILED,
                         "sip raw data failed");
  }
...
srs_error_t SrsGb28181SipService::send_ptz(SrsSipRequest *req, std::string chid,
  ...
  if (device->invite_status != SrsGb28181SipSessionInviteOk) {
    return srs_error_new(ERROR_GB28181_SIP_NOT_INVITE,
                         "sip device channel not inviting");
  }
  ...
  if (!strcasecmp(ss_cmd, "stop")) {
    ...
    return srs_error_new(ERROR_GB28181_SIP_PTZ_CMD_INVALID,
                         "sip ptz cmd no support");
  }

  if (speed < 0 || speed > 0xFF) {
    return srs_error_new(ERROR_GB28181_SIP_PTZ_CMD_INVALID,
                         "sip ptz cmd speed out of range");
  }
...
srs_error_t SrsGb28181SipService::query_sip_session(std::string sid,
                                                    SrsJsonArray *arr) {
  srs_error_t err = srs_success;
...
srs_error_t SrsGb28181SipService::query_device_list(std::string sid,
                                                    SrsJsonArray *arr) {
  srs_error_t err = srs_success;
...
srs_error_t SrsGb28181SipService::fetch_or_create_sip_session(
    SrsSipRequest *req, SrsGb28181SipSession **sip_session) {
  srs_error_t err = srs_success;
  ...
  if ((err = sess->serve()) != srs_success) {
    return srs_error_wrap(err, "gb28181: sip serssion serve %s",
                          req->sip_auth_id.c_str());
  }
...
void SrsGb28181SipService::sip_session_map_by_callid(SrsGb28181SipSession *sess,
                                                     std::string call_id) {
...
void SrsGb28181SipService::sip_session_unmap_by_callid(std::string call_id) {
  std::map<std::string, SrsGb28181SipSession *>::iterator it =
      sessions_by_callid.find(call_id);
  if (it != sessions_by_callid.end()) {
    sessions_by_callid.erase(it);
  }
}

SrsGb28181SipSession *SrsGb28181SipService::fetch_session_by_callid(
    std::string call_id) {
