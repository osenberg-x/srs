...
class CodeConverter {
 ...
 public:
  CodeConverter(const char *pFromCharset, const char *pToCharset) {
    ...
    if ((iconv_t)(-1) == cd)
      srs_warn("CodeConverter: iconv_open failed <%s --> %s>", pFromCharset,
               pToCharset);
  }
  ~CodeConverter() {
    if ((iconv_t)(-1) != cd) iconv_close(cd);
  }
...
std::string srs_sip_get_utc_date() {
  ...
  std::string utc_date = "";
  char buffer[25] = {0};
  snprintf(buffer, 25, "%d-%02d-%02dT%02d:%02d:%02d.%03d", 1900 + tm->tm_year,
...
std::string srs_sip_get_param(std::string msg, std::string param) {
  ...
  for (vector<string>::iterator it = vec_params.begin(); it != vec_params.end();
       ++it) {
    string value = *it;
...
bool SrsSipRequest::is_register() { return method == SRS_SIP_METHOD_REGISTER; }

bool SrsSipRequest::is_invite() { return method == SRS_SIP_METHOD_INVITE; }

bool SrsSipRequest::is_ack() { return method == SRS_SIP_METHOD_ACK; }

bool SrsSipRequest::is_message() { return method == SRS_SIP_METHOD_MESSAGE; }

bool SrsSipRequest::is_bye() { return method == SRS_SIP_METHOD_BYE; }
...
SrsSipStack::SrsSipStack() { buf = new SrsSimpleStream(); }

SrsSipStack::~SrsSipStack() { srs_freep(buf); }

srs_error_t SrsSipStack::parse_request(SrsSipRequest **preq,
                                       const char *recv_msg, int nb_len) {
...
srs_error_t SrsSipStack::parse_xml(
    std::string xml_msg, std::map<std::string, std::string> &json_map,
...
*/
  ...
  if (CodeConverter::IsUTF8(xml_msg.c_str(), xml_msg.size()) == false) {
    ...
    if (cc.Convert((char *)xml_msg.c_str(), xml_msg.size(), (char *)outBuf,
                   xml_msg.size() * 2 - 1) != -1)
    ...
    if (p[0] == '\n') {
      ...
      */
      ...
      if (in_item_tag && value != "" && (xml_layer - in_item_tag) == 2) {
        // (xml_layer - in_item_tag) == 2, this condition filters all deeper
        // tags under tag "Item"
        one_item[key] = value;
      }
...
srs_error_t SrsSipStack::do_parse_request(SrsSipRequest *req,
                                          const char *recv_msg) {
  srs_error_t err = srs_success;

  std::vector<std::string> header_body =
      srs_string_split(recv_msg, SRS_RTSP_CRLFCRLF);
  if (header_body.empty()) {
    return srs_error_new(ERROR_GB28181_SIP_PRASE_FAILED,
                         "parse reques message");
  }
  ...
  if (header_body.size() > 1) {
    ...
    body = recv_str.substr(recv_str.find(SRS_RTSP_CRLFCRLF) +
                           strlen(SRS_RTSP_CRLFCRLF));
      ...
      if (firstline == "") {
        ...
        if (pos != string::npos) {
          if (pos != 0) {
            ...
            std::string content =
                oneline.substr(pos + 1, oneline.length() - pos - 1);
            ...
            if (!strcasecmp(phead, "call-id:")) {
              ...
              // TODO: fixme
              srs_trace("sip: unkonw message head %s content=%s", phead,
                        content.c_str());
            }
          }
        }
  ...
  if (method_uri_ver.empty() || method_uri_ver.size() < 3) {
    return srs_error_new(
        ERROR_GB28181_SIP_PRASE_FAILED,
        "parse request firstline is empty or less than 3 fields");
  }
  ...
  } else {  // request first line text :MESSAGE
  ...
  if (!strcasecmp(req->content_type.c_str(), "application/manscdp+xml")) {
    // xml to map
    if ((err = parse_xml(body, req->xml_body_map, req->item_list)) !=
        srs_success) {
    ...
    if (req->xml_body_map.find("Response") != req->xml_body_map.end()) {
      ...
      if (cmdtype == "Catalog") {
        ...
        for (int i = 0; i < (int)req->item_list.size(); i++) {
          ...
          std::string status;
          if (one_item.find("Status") != one_item.end() &&
              one_item.find("Name") != one_item.end()) {
      ...
      if (cmdtype == "Keepalive") {
        // TODO: ????
        std::vector<std::string> vec_device_id =
            srs_string_split(req->xml_body_map["Notify@Info@DeviceID"], ",");
        for (int i = 0; i < (int)vec_device_id.size(); i++) {
    ...
    std::vector<std::string> sdp_lines =
        srs_string_split(body.c_str(), SRS_RTSP_CRLF);
  ...
  srs_info("sip: method=%s uri=%s version=%s cmdtype=%s", req->method.c_str(),
           req->uri.c_str(), req->version.c_str(),
           req->get_cmdtype_str().c_str());
...
void SrsSipStack::resp_keepalive(std::stringstream &ss, SrsSipRequest *req) {
  ss << SRS_SIP_VERSION << " 200 OK" << SRS_RTSP_CRLF
     << "Via: " << SRS_SIP_VERSION << "/UDP " << req->host << ":"
     << req->host_port << ";branch=" << req->branch << SRS_RTSP_CRLF
     << "From: <sip:" << req->from.c_str() << ">;tag=" << req->from_tag
     << SRS_RTSP_CRLF << "To: <sip:" << req->to.c_str() << ">\r\n"
     << "Call-ID: " << req->call_id << SRS_RTSP_CRLF << "CSeq: " << req->seq
     << " " << req->method << SRS_RTSP_CRLF << "Contact: " << req->contact
     << SRS_RTSP_CRLF << "Max-Forwards: 70" << SRS_RTSP_CRLF
  ...
  if (req->method == "REGISTER") {
    ...
    */
    if (req->authorization.empty()) {
      srs_trace(
          "gb28181: send 401 unauthorized for %s register authentication.",
    ...
    ss << SRS_SIP_VERSION << " 200 OK" << SRS_RTSP_CRLF
       ...
       << "To: " << get_sip_to(req) << SRS_RTSP_CRLF << "CSeq: " << req->seq
       << " " << req->method << SRS_RTSP_CRLF << "Call-ID: " << req->call_id
       << SRS_RTSP_CRLF << "Contact: " << req->contact << SRS_RTSP_CRLF
       << "User-Agent: " << SRS_SIP_USER_AGENT << SRS_RTSP_CRLF
       << "Expires: " << req->expires << SRS_RTSP_CRLF << "Content-Length: 0"
       << SRS_RTSP_CRLFCRLF;
  } else {
    /*
    ...
    */
    ...
    ss << SRS_SIP_VERSION << " 200 OK" << SRS_RTSP_CRLF
       ...
       << "To: " << get_sip_to(req) << SRS_RTSP_CRLF << "CSeq: " << req->seq
       << " " << req->method << SRS_RTSP_CRLF << "Call-ID: " << req->call_id
       << SRS_RTSP_CRLF << "User-Agent: " << SRS_SIP_USER_AGENT << SRS_RTSP_CRLF
       << "Content-Length: 0" << SRS_RTSP_CRLFCRLF;
  }
}

void SrsSipStack::req_invite(stringstream &ss, SrsSipRequest *req, string ip,
  ...
  */
  ...
  if (!tcpFlag) {
    sdp << "v=0" << SRS_RTSP_CRLF << "o=" << req->serial << " 0 0 IN IP4 " << ip
        << SRS_RTSP_CRLF << "s=Play" << SRS_RTSP_CRLF << "c=IN IP4 " << ip
        << SRS_RTSP_CRLF << "t=0 0"
        << SRS_RTSP_CRLF
        // TODO 97 98 99 current no support
        //<< "m=video " << port <<" RTP/AVP 96 97 98 99" << SRS_RTSP_CRLF
        << "m=video " << port << " RTP/AVP 96" << SRS_RTSP_CRLF << "a=recvonly"
        << SRS_RTSP_CRLF << "a=rtpmap:96 PS/90000"
        << SRS_RTSP_CRLF
    ...
    sdp << "v=0" << SRS_RTSP_CRLF << "o=" << req->serial << " 0 0 IN IP4 " << ip
        ...
        << "a=recvonly" << SRS_RTSP_CRLF << "a=rtpmap:96 PS/90000"
        << SRS_RTSP_CRLF << "a=setup:passive" << SRS_RTSP_CRLF
        << "a=connection:new"
        << SRS_RTSP_CRLF
        // TODO: current no support
        //<< "a=rtpmap:97 MPEG4/90000" << SRS_RTSP_CRLF
  ...
  ss << "INVITE " << req->uri << " " << SRS_SIP_VERSION << SRS_RTSP_CRLF
     << "Via: " << SRS_SIP_VERSION << "/UDP " << req->host << ":"
     << req->host_port << ";rport;branch=" << req->branch << SRS_RTSP_CRLF
     << "From: " << get_sip_from(req) << SRS_RTSP_CRLF
     << "To: " << get_sip_to(req) << SRS_RTSP_CRLF
     << "Call-ID: " << req->call_id << SRS_RTSP_CRLF << "CSeq: " << req->seq
     << " INVITE" << SRS_RTSP_CRLF << "Content-Type: Application/SDP"
     << SRS_RTSP_CRLF << "Contact: <sip:" << req->to << ">" << SRS_RTSP_CRLF
     << "Max-Forwards: 70" << SRS_RTSP_CRLF
     << "User-Agent: " << SRS_SIP_USER_AGENT << SRS_RTSP_CRLF
     << "Subject: " << req->chid << ":" << _ssrc << "," << req->serial << ":0"
     << SRS_RTSP_CRLF << "Content-Length: " << sdp.str().length()
     << SRS_RTSP_CRLFCRLF << sdp.str();
}

void SrsSipStack::req_401_unauthorized(std::stringstream &ss,
                                       SrsSipRequest *req) {
  /* sip-agent <-----401 Unauthorized ------ sip-server
  SIP/2.0 401 Unauthorized
  Via: SIP/2.0/UDP
  192.168.137.92:5061;rport=61378;received=192.168.1.13;branch=z9hG4bK802519080
  ...
  realm="3402000000",qop="auth",nonce="f1da98bd160f3e2efe954c6eedf5f75a"
  */

  ss << SRS_SIP_VERSION << " 401 Unauthorized"
     << SRS_RTSP_CRLF
     ...
     << "WWW-Authenticate: Digest "
        "realm=\"3402000000\",qop=\"auth\",nonce="
        "\"f1da98bd160f3e2efe954c6eedf5f75a\""
     << SRS_RTSP_CRLFCRLF;
  return;
}
...
void SrsSipStack::req_ack(std::stringstream &ss, SrsSipRequest *req) {
  ...
  */
  ...
  ss << "ACK "
     << "sip:" << req->chid << "@" << req->realm << " " << SRS_SIP_VERSION
     << SRS_RTSP_CRLF << "Via: " << SRS_SIP_VERSION << "/UDP " << req->host
     << ":" << req->host_port << ";rport;branch=" << req->branch
     << SRS_RTSP_CRLF << "From: " << get_sip_from(req) << SRS_RTSP_CRLF
     << "To: " << get_sip_to(req) << SRS_RTSP_CRLF
     << "Call-ID: " << req->call_id << SRS_RTSP_CRLF << "CSeq: " << req->seq
     << " ACK" << SRS_RTSP_CRLF << "Max-Forwards: 70" << SRS_RTSP_CRLF
     << "User-Agent: " << SRS_SIP_USER_AGENT << SRS_RTSP_CRLF
     << "Content-Length: 0" << SRS_RTSP_CRLFCRLF;
}
...
void SrsSipStack::req_bye(std::stringstream &ss, SrsSipRequest *req) {
  ...
  */
  ...
  ss << "BYE " << req->uri << " " << SRS_SIP_VERSION
     << SRS_RTSP_CRLF
     //<< "Via: "<< SRS_SIP_VERSION << "/UDP "<< req->host << ":" <<
     // req->host_port << ";rport" << branch << SRS_RTSP_CRLF
     << "Via: " << SRS_SIP_VERSION << "/UDP " << req->host << ":"
     << req->host_port << ";rport;branch=" << req->branch << SRS_RTSP_CRLF
     << "From: " << get_sip_from(req) << SRS_RTSP_CRLF
     << "To: " << get_sip_to(req)
     ...
     << "Call-ID: " << req->call_id << SRS_RTSP_CRLF << "CSeq: " << req->seq
     << " BYE" << SRS_RTSP_CRLF << "Max-Forwards: 70" << SRS_RTSP_CRLF
     << "User-Agent: " << SRS_SIP_USER_AGENT << SRS_RTSP_CRLF
     << "Content-Length: 0" << SRS_RTSP_CRLFCRLF;
}
...
void SrsSipStack::req_query_catalog(std::stringstream &ss, SrsSipRequest *req) {
  ...
  SIP/2.0 200 OK
  Via: SIP/2.0/UDP
  192.168.1.17:5060;rport=5060;received=192.168.1.17;branch=z9hG4bK563315752
  From: <sip:34020000001110000001@3402000000>;tag=387315752
  To: <sip:34020000001110000001@192.168.1.21:5060>;tag=1420696981
  CSeq: 32 MESSAGE
  Call-ID: 728315752
  ...
  */
  ...
  xml << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" << SRS_RTSP_CRLF
      << "<Query>" << SRS_RTSP_CRLF << "<CmdType>Catalog</CmdType>"
      << SRS_RTSP_CRLF << "<SN>" << srs_sip_generate_sn() << "</SN>"
      << SRS_RTSP_CRLF << "<DeviceID>" << req->sip_auth_id << "</DeviceID>"
      << SRS_RTSP_CRLF << "</Query>" << SRS_RTSP_CRLF;
  xmlbody = xml.str();
  ...
  ss << "MESSAGE " << req->uri << " " << SRS_SIP_VERSION << SRS_RTSP_CRLF
     ...
     << "To: " << get_sip_to(req) << SRS_RTSP_CRLF
     << "Call-ID: " << req->call_id << SRS_RTSP_CRLF << "CSeq: " << req->seq
     << " MESSAGE" << SRS_RTSP_CRLF << "Content-Type: Application/MANSCDP+xml"
     << SRS_RTSP_CRLF << "Max-Forwards: 70" << SRS_RTSP_CRLF
     << "User-Agent: " << SRS_SIP_USER_AGENT << SRS_RTSP_CRLF
     << "Content-Length: " << xmlbody.length() << SRS_RTSP_CRLFCRLF << xmlbody;
}

void SrsSipStack::req_ptz(std::stringstream &ss, SrsSipRequest *req,
                          uint8_t cmd, uint8_t speed, int priority) {
  /*
  <?xml version="1.0"?>
  <Control>
  ...
  */
  ...
  xml << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" << SRS_RTSP_CRLF
      << "<Control>" << SRS_RTSP_CRLF << "<CmdType>DeviceControl</CmdType>"
      << SRS_RTSP_CRLF << "<SN>" << srs_sip_generate_sn() << "</SN>"
      << SRS_RTSP_CRLF << "<DeviceID>" << req->sip_auth_id << "</DeviceID>"
      << SRS_RTSP_CRLF << "<PTZCmd>" << ss_ptzcmd.str() << "</PTZCmd>"
      << SRS_RTSP_CRLF << "<Info>" << SRS_RTSP_CRLF << "<ControlPriority>"
      << priority << "</ControlPriority>" << SRS_RTSP_CRLF << "</Info>"
      << SRS_RTSP_CRLF << "</Control>" << SRS_RTSP_CRLF;
  xmlbody = xml.str();
  ...
  ss << "MESSAGE " << req->uri << " " << SRS_SIP_VERSION
     << SRS_RTSP_CRLF
     //<< "Via: "<< SRS_SIP_VERSION << "/UDP "<< req->host << ":" <<
     // req->host_port << ";rport" << branch << SRS_RTSP_CRLF
     << "Via: " << SRS_SIP_VERSION << "/UDP " << req->host << ":"
